...
tag_r2=0.8
n_vcf_parallel=120
N=120 #at a time how many exposure should run
...
parser$add_argument("--cis_mhc_remove",
                    help = "Specify whether to remove MHC region or not from cis pqtls",
                    choices = c("Yes", "No"),
                    default = "No")

parser$add_argument("--trans_mhc_remove",
                    help = "Specify whether to remove MHC region or not from trans pqtls",
                    choices = c("Yes", "No"),
                    default = "No")
...
cis_mhc_remove<- args$cis_mhc_remove
trans_mhc_remove<- args$trans_mhc_remove
    ...
    if (length(conditions) > 0) {
        ...
        if (tolower(indels) == "no") {
            if (variant_id_type == "unique_id") {
                command <- glue(" bcftools annotate --set-id {id}  {vcf_file}  | bcftools view --types snps --include '{filter_condition}' | bcftools sort  --temp-dir {output_folder} | bgzip -c > {vcf_file_out}")
            } else {
                command <- glue("bcftools view --types snps --include '{filter_condition}' {vcf_file} | bcftools sort  --temp-dir {output_folder} | bgzip -c > {vcf_file_out}")
            }
        } else {
            if (variant_id_type == "unique_id") {
                command <- glue(" bcftools annotate --set-id {id} {vcf_file}  | bcftools view --include '{filter_condition}' {vcf_file} | bcftools sort  --temp-dir  {output_folder} | bgzip -c > {vcf_file_out}")
            } else {
                command <- glue("bcftools view --include '{filter_condition}' {vcf_file} | bcftools sort --temp-dir {output_folder} | bgzip -c > {vcf_file_out}")
            }
        }
...
read_exposure_vcf_and_process <- function(exposure_id, dbsnp_df, ld_file, infoscore_cutoff, maf_cutoff, pvalue_cutoff, 
                                         variant_id_type, cis_mhc_remove, trans_mhc_remove, mhc_region, remove_chrX, exposure_file_df) {
  
    # Initialize an empty list to store results
    cis_trans_list <- list()
    
    # Extract MHC region parameters
    mhc_region <- strsplit(mhc_region, "[:-]")[[1]]
    mhc_chr <- as.character(mhc_region[1])
    mhc_start <- as.numeric(mhc_region[2])
    mhc_end <- as.numeric(mhc_region[3])
    
    # Extract gene information based on exposure_id
    gene_start <- as.numeric(exposure_file_df[exposure_file_df$gwas_id == exposure_id, "gene_start"])
    gene_chr <- as.character(exposure_file_df[exposure_file_df$gwas_id == exposure_id, "chr"])
    
    # Define cis_start and cis_end based on gene_start
    cis_start <- pmax(gene_start - 1000000, 1)
    cis_end <- gene_start + 1000000
    
    # Read VCF file
    tryCatch({
        vcf <- readVcf(glue("{output_folder}filtered_exposure_vcf/{exposure_id}_filt.vcf.gz"))
        df <- as.data.frame(vcf_to_granges(vcf) %>% as_tibble())
    }, 
    error = function(e) {
        message(glue("Error reading VCF file for sample '{exposure_id}': {conditionMessage(e)}"))
        return(cis_trans_list)  # Return empty list on error
    })
    ...
    if (exists("df") && nrow(df) > 0) {
        ...
        df <- df[complete.cases(df[, c("LP", "start", "ES", "SE")]), ]
        
        # Merge with dbSNP data if provided
        if (!is.null(dbsnp_df) && !identical(dbsnp_df, "NA")) {
            df <- merge(df, dbsnp_df, by = c('seqnames', 'start', 'REF', 'ALT'), all.x = TRUE)
            } else {
        ...
        if ("AF" %in% colnames(df) && palindromic == 'Yes') {
        ...
        df <- df[!(df$seqnames %in% c("Y", "MT")), ]
        message(glue("Removing variants present in Y and MT chromosome for sample '{exposure_id}'"))
        
        # Split into CIS and TRANS variants
        df$variant_type <- ifelse(df$seqnames == gene_chr & df$start >= cis_start & df$start <= cis_end, "CIS", "TRANS")
        
        cis_df <- df[df$variant_type == 'CIS', ]
        trans_df <- df[df$variant_type == 'TRANS', ]
        
        # Filter based on MHC regions
        if (cis_mhc_remove == "Yes" && nrow(cis_df) > 0) {
            cis_df <- cis_df[!(cis_df$seqnames == mhc_chr & cis_df$start > mhc_start & cis_df$start < mhc_end), ]
            message(glue("Removing variants present in the MHC region for sample '{exposure_id}' (CIS Variants): MHC Region was {mhc_chr}:{mhc_start}-{mhc_end}"))
            } else {
            message(glue("Not removing variants from the MHC regions for sample '{exposure_id}' (CIS Variants)"))
        }
        
        if (trans_mhc_remove == "Yes" && nrow(trans_df) > 0) {
            trans_df <- trans_df[!(trans_df$seqnames == mhc_chr & trans_df$start > mhc_start & trans_df$start < mhc_end), ]
            message(glue("Removing variants present in the MHC region for sample '{exposure_id}' (TRANS Variants): MHC Region was {mhc_chr}:{mhc_start}-{mhc_end}"))
            } else {
            message(glue("Not removing variants from the MHC regions for sample '{exposure_id}' (TRANS Variants)"))
        }
        
        cis_trans_list$cis <- cis_df
        cis_trans_list$trans <- trans_df
        
    } else {
        message(glue("No variants found or error occurred for sample '{exposure_id}'"))
    }
    
    return(cis_trans_list)
}
...
##This function perform All the MR analysis 
runMRAnalyses <- function(exposure_id, exposure_df, variant_type, outcome_file_df, 
                         local_exposure_with_no_significant_variants_after_ld_prune) {
  file_suffixes <- c("Harmonised_Exposure_Outcome", 'MendelianPipeline_Test', 'MendelianRandomization_AllTest',
                     'MendelianRandomization_IVW_Delta_Test', 'TwoSampleMR_Analysis_Directionality_Test',
                     'TwoSampleMR_Analysis_Heterogeneity_Test', 'TwoSampleMR_Analysis_Hpleiotropy_Test',
  ...
  summary_folder=glue("{output_folder}/summary_exposure_outcome/")
  system(glue("mkdir -p {summary_folder}"))
  local_qc_passed_exposure_variants_failed_during_ld_prune_df <- data.frame()
  local_qc_passed_exposure_variants_after_ld_prune_df <- data.frame()

  tryCatch({
    ld_pruned <- ld_pruning(exposure_id, exposure_df, ld_file, local_qc_passed_exposure_variants_failed_during_ld_prune_df, local_qc_passed_exposure_variants_after_ld_prune_df)
  }, error = function(e) {
    message(glue("LD pruning failed for '{exposure_id}', using all the exposure variants: {conditionMessage(e)}"))
  })

  exposure_df <- ld_pruned$selected_df
  if (nrow(ld_pruned$local_qc_passed_exposure_variants_after_ld_prune_df) > 0) {
    write.csv(ld_pruned$local_qc_passed_exposure_variants_after_ld_prune_df, glue('{summary_folder}{exposure_id}_{variant_type}_qc_passed_exposure_variants_after_ld_prune.csv'), row.names = FALSE)
  }
  if (nrow(ld_pruned$local_qc_passed_exposure_variants_failed_during_ld_prune_df) > 0) {
    write.csv(ld_pruned$local_qc_passed_exposure_variants_failed_during_ld_prune_df, glue('{summary_folder}{exposure_id}_{variant_type}_qc_passed_exposure_variants_failed_during_ld_prune.csv'), row.names = FALSE)
  }
  ...
  if (nrow(exposure_df) != 0) {
    for (outcome_id in outcome_file_df$gwas_id) {
      ...
      results <- list()
      out_folder <- glue("{output_folder}{exposure_id}/")
      system(glue("mkdir -p {out_folder}"))
      outcomevcf <- glue("{output_folder}/filtered_outcome_vcf/{outcome_id}_filt.vcf.gz")
      prefix <- glue("{out_folder}{exposure_id}_{outcome_id}_{variant_type}")
      results <- Harmonise_exposure_Outcome_data(outcome_id, exposure_id, exposure_df, outcomevcf, 
                                                  ld_file, pvalue_cutoff, tag_r2,
                                                  local_outcome_with_no_variant_df,
                                                  local_harmonised_df)
    ...
    for (suffix in file_suffixes) {
      m_df <- data.frame()
      for (outcome_id in outcome_file_df$gwas_id) {
        file_path <- glue("{out_folder}{exposure_id}_{outcome_id}_{variant_type}_{suffix}.csv")
        # Attempt to read the CSV file and concatenate to m_df
        tryCatch({
          if (file.exists(file_path)) {
            df <- read.csv(file_path, sep = ",")
      ...
      tryCatch({
        if (suffix != "Harmonised_Exposure_Outcome") {
          write.csv(m_df, glue('{out_folder}{exposure_id}_{variant_type}_with_all_outcomes_{suffix}.csv'), row.names = FALSE)
        } else {
          write.csv(m_df, glue('{summary_folder}{exposure_id}_{variant_type}_with_all_outcomes_{suffix}.csv'), row.names = FALSE)
        }
      }, error = function(e) { 
        cat("Error writing merged data frame to CSV:", conditionMessage(e), "\n") 
      })
  ...
  return(local_exposure_with_no_significant_variants_after_ld_prune)
}


##This function will create all the inputs MR analysis and Call runMRAnalyses function 
#c("samplesize.outcome",'ncase.outcome','ncontrol.outcome','samplesize.exposure')
createMRInputs <- function(exposure_part_df, part, outcome_file_df, output_folder, dbsnp_df, ld_file, 
                                              infoscore_cutoff, maf_cutoff, pvalue_cutoff, variant_id_type,mhc_remove, 
                                              mhc_region, remove_chrX,outcomevcf_path, tag_r2) {

    exposure_with_no_significant_variants_before_ld_prune <- character()
    cis_exposure_with_no_significant_variants_before_ld_prune <- character()
    trans_exposure_with_no_significant_variants_before_ld_prune <- character()
    cis_exposure_with_no_significant_variants_after_ld_prune<- character()
    trans_exposure_with_no_significant_variants_after_ld_prune<- character()
    ...
    for (exposure_id in exposure_part_df$gwas_id) {
        ...
        if (variant_count > 0) {
            ...
            cist_trans<-read_exposure_vcf_and_process(exposure_id, dbsnp_df, ld_file, infoscore_cutoff, 
                                                     maf_cutoff, pvalue_cutoff, variant_id_type, cis_mhc_remove, 
                                                     trans_mhc_remove, mhc_region, remove_chrX, exposure_file_df)
            cis_df<-cist_trans$cis
            trans_df<-cist_trans$trans

            if (nrow(cis_df) > 0) {
                    write.csv(cis_df, glue('{summary_folder}{exposure_id}_CIS_qc_passed_exposure_variants_before_ld_prune.csv'), row.names = FALSE)
                    cis_exposure_with_no_significant_variants_after_ld_prune<-runMRAnalyses(exposure_id,cis_df,"CIS",outcome_file_df,cis_exposure_with_no_significant_variants_after_ld_prune)
                }else{
                    cis_exposure_with_no_significant_variants_before_ld_prune<-c(cis_exposure_with_no_significant_variants_before_ld_prune,exposure_id)
            }
                
            if (nrow(trans_df) > 0) {
                    write.csv(trans_df, glue('{summary_folder}{exposure_id}_TRANS_qc_passed_exposure_variants_before_ld_prune.csv'), row.names = FALSE)
                    trans_exposure_with_no_significant_variants_after_ld_prune<-runMRAnalyses(exposure_id,trans_df,"TRANS",outcome_file_df,trans_exposure_with_no_significant_variants_after_ld_prune)
                }else{
                    trans_exposure_with_no_significant_variants_before_ld_prune<-c(trans_exposure_with_no_significant_variants_before_ld_prune,exposure_id)
            }

        }else{
            exposure_with_no_significant_variants_before_ld_prune <- c(exposure_with_no_significant_variants_before_ld_prune,exposure_id)
        }
    }
    write.csv(as.data.frame(exposure_with_no_significant_variants_before_ld_prune), glue('{summary_folder}Exposure_With_No_qc_passed_variants_before_ld_prune_in_Both_CIS_TRANS_Part{part}.csv'), row.names = FALSE)
    write.csv(as.data.frame(cis_exposure_with_no_significant_variants_before_ld_prune), glue('{summary_folder}Exposure_With_No_qc_passed_variants_before_ld_prune_in_CIS_Part{part}.csv'), row.names = FALSE)
    write.csv(as.data.frame(trans_exposure_with_no_significant_variants_before_ld_prune), glue('{summary_folder}Exposure_With_No_qc_passed_variants_before_ld_prune_in_TRANS_Part{part}.csv'), row.names = FALSE)
    write.csv(as.data.frame(cis_exposure_with_no_significant_variants_after_ld_prune), glue('{summary_folder}Exposure_With_No_qc_passed_variants_after_ld_prune_in_CIS_Part{part}.csv'), row.names = FALSE)
    write.csv(as.data.frame(trans_exposure_with_no_significant_variants_after_ld_prune), glue('{summary_folder}Exposure_With_No_qc_passed_variants_after_ld_prune_in_TRANS_Part{part}.csv'), row.names = FALSE)
}


##This will per form ALL MR Input file preparation and MR analysis
run_parallel_mendelian_analysis_pipeline <- function(exposure_file_df, outcome_file_df, output_folder, dbsnp_df, ld_file, 
    ...
    mclapply(1:length(job_batches), function(i) {
        createMRInputs(job_batches[[i]], i, outcome_file_df, output_folder, dbsnp_df, ld_file, 
                                        infoscore_cutoff, maf_cutoff, pvalue_cutoff, variant_id_type, mhc_remove, 
                                        mhc_region, remove_chrX, outcomevcf_path, tag_r2) }, mc.cores = n_vcf_parallel)
    

    summary_folder <- glue("{output_folder}/summary_exposure_outcome/")

    merge_and_save_csv <- function(pattern, folder_name) {
        file_paths <- list.files(path = folder_name, pattern = pattern, full.names = TRUE, recursive = TRUE)
        
        if (length(file_paths) > 0) {
            output_file <- glue("{folder_name}/{pattern}_merged.csv")
            
            # Initialize the output file
            if (file.exists(output_file)) file.remove(output_file)
            
            merged_list <- list()  # To store data frames for merging

            for (file in file_paths) {
                file_info <- try(file.info(file), silent = TRUE)
                
                if (inherits(file_info, "try-error")) {
                    message(glue("Error getting file info for {file}. Skipping file."))
                    next
                }
                
                if (is.na(file_info$size) || file_info$size == 0) {
                    message(glue("File {file} is empty or does not exist. Skipping file."))
                    next
                }
                tryCatch({
                    df <- fread(file)
                    ...
                    # Collect data frames
                    merged_list <- append(merged_list, list(df))
                    
                }, error = function(e) {
                    message(glue("Error reading file {file}: {e$message}. Skipping file."))
                })
            }
            if (length(merged_list) > 0) {
                merged_df <- rbindlist(merged_list,fill=TRUE)
                # Remove V1 column if it exists
                if ("V1" %in% names(merged_df)) {
                    merged_df <- merged_df %>% select(-V1)
                }

                merged_df <- merged_df %>%filter(!if_all(everything(), is.na))
                # Check if pattern contains Heterogeneity_Test and apply pivoting
                if (grepl("Heterogeneity_Test", pattern)) {
                    merged_df <- merged_df %>%
                        mutate(across(c(Q, Q_df, Q_pval), as.numeric)) %>%
                        pivot_wider(names_from = method, values_from = c(Q, Q_df, Q_pval), names_glue = "{.value}_{method}")
                }

                # Write the mergd data frame to the output file
                fwrite(merged_df, file = output_file)
                message(glue("Merged and saved files for pattern: {pattern}"))
            } else {
                message(glue("No valid data found in files for pattern: {pattern}"))
            }
            
            # Free up memory
            rm(merged_list, merged_df)
            gc()
        } else {
    ...
    summary_patterns <- c('Exposure_With_No_qc_passed_variants_before_ld_prune_in_Both_CIS_TRANS','Exposure_With_No_qc_passed_variants_before_ld_prune_in_CIS',
                        'Exposure_With_No_qc_passed_variants_before_ld_prune_in_TRANS','Exposure_With_No_qc_passed_variants_after_ld_prune_in_CIS','Exposure_With_No_qc_passed_variants_after_ld_prune_in_TRANS',
                        'TRANS_qc_passed_exposure_variants_before_ld_prune','CIS_qc_passed_exposure_variants_before_ld_prune','TRANS_qc_passed_exposure_variants_after_ld_prune',
                        'CIS_qc_passed_exposure_variants_after_ld_prune','TRANS_with_all_outcomes_Harmonised_Exposure_Outcome','CIS_with_all_outcomes_Harmonised_Exposure_Outcome')

    # Original TRANS_ patterns
    trans_patterns <- c(
        'TRANS_TwoSampleMR_Analysis_SingleVariantMetafixed_Test.csv','TRANS_TwoSampleMR_Analysis_SingleVariantWald_Test.csv',
        'TRANS_TwoSampleMR_Analysis_Multiple_MR_Test.csv','TRANS_MendelianRandomization_IVW_Delta_Test.csv','TRANS_MendelianRandomization_AllTest.csv',
        'TRANS_MendelianPipeline_Test.csv','TRANS_TwoSampleMR_Analysis_Hpleiotropy_Test.csv','TRANS_TwoSampleMR_Analysis_Heterogeneity_Test.csv',
        'TRANS_TwoSampleMR_Analysis_Directionality_Test.csv')
    # Generate CIS_ patterns by replacing TRANS_ with CIS_
    cis_patterns <- gsub("^TRANS_", "CIS_", trans_patterns)
    # Combine both sets of patterns
    all_patterns <- c(trans_patterns, cis_patterns)

    # Apply the function to each pattern
    lapply(summary_patterns, function(pattern) merge_and_save_csv(pattern, summary_folder))
    lapply(all_patterns, function(pattern) merge_and_save_csv(pattern, output_folder))
    }


##Merge all the results IVD delta with all sensitivity analsis
compile_final_results <- function(output_folder, prefix) {
    # Read the initial three CSV files
    file1 <- fread(file.path(output_folder, glue("{prefix}_TwoSampleMR_Analysis_Hpleiotropy_Test.csv_merged.csv")))
    columns_to_rename <- c("egger_intercept", "se", "pval")
    setnames(file1, old = columns_to_rename, new = paste0("Hpleiotropy_", columns_to_rename))

    file2 <- fread(file.path(output_folder, glue("{prefix}_TwoSampleMR_Analysis_Directionality_Test.csv_merged.csv")))
    columns_to_rename <-c("snp_r2.exposure" ,"snp_r2.outcome","correct_causal_direction" ,"steiger_pval")
    setnames(file2, old = columns_to_rename, new = paste0("Directionality_", columns_to_rename))

    file3 <- fread(file.path(output_folder, glue("{prefix}_TwoSampleMR_Analysis_Heterogeneity_Test.csv_merged.csv")))
    columns_to_rename<-c("Q_Maximum likelihood","Q_MR Egger","Q_Inverse variance weighted","Q_IVW radial","Q_Unweighted regression",
                        "Q_df_Maximum likelihood","Q_df_MR Egger","Q_df_Inverse variance weighted","Q_df_IVW radial", "Q_df_Unweighted regression",
                        "Q_pval_Maximum likelihood","Q_pval_MR Egger","Q_pval_Inverse variance weighted","Q_pval_IVW radial","Q_pval_Unweighted regression"   )
    setnames(file3, old = columns_to_rename, new = paste0("Heterogeneity_", columns_to_rename))

    # Merge based on common columns
    merged_df <- merge(file1, file2, by = c('id.exposure', 'id.outcome', 'exposure', 'outcome'), all = TRUE)
    merged_df <- merge(merged_df, file3, by = c('id.exposure', 'id.outcome', 'exposure', 'outcome'), all = TRUE)
    # Read the fourth CSV file
    file4 <- fread(file.path(output_folder, glue("{prefix}_MendelianRandomization_IVW_Delta_Test.csv_merged.csv")))
    # Rename columns for consistency in merging
    setnames(file4, old = c("Outcome", "Exposure"), new = c("outcome", "exposure"))
    # Merge with merged_df based on 'exposure' and 'outcome' columns
    merged_df <- merge(file4,merged_df, by = c('exposure', 'outcome'), all = TRUE)
    # Define output file path
    output_file <- file.path(output_folder, glue("{prefix}_MendelianRandomization_IVW_Delta_Test_withSensitivity_Test.csv"))
    # Write merged dataframe to CSV
    fwrite(merged_df, file = output_file, row.names = FALSE)
}
...
compile_final_results(output_folder, "TRANS")
compile_final_results(output_folder, "CIS")
